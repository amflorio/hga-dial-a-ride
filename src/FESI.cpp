...
// Florio: adapted to read from Instance object instead of file
void initLocation() {
    nV=I->nodes();
    initMemory(nV);
}
...
// Florio: adapted to generate unique filename per instance
string getHSTFileName(string& des, int id) {
    sprintf(buf, "%s/hst_%s_%lu_%lu_%lu_%d.txt", des.c_str(), I->code().c_str(),
            I->requests().size(), I->drivers().size(), I->capacity(), id);
    return string(buf);
}
...
// Florio: adapted to read from Instance object instead of file
void trainHST(string des) {
    const int MAX_SAMPLE = 10;
    initLocation();
    for(int i=0; i<MAX_SAMPLE; ++i) {
        string desFileName = getHSTFileName(des, i);
        clock_t t0 = clock(), t1;
        constructHST(false);
        t1 = clock();
...
vector<Route> FESI() {
    ...
    // Florio: adapt original code to return a solution in our representation
    vector<Route> routes;
    double tot_cost=0;
    for (size_t k=0; k<nW; ++k) {
        tot_cost+=getRouteDist(k);
        Route rt(workers[k].oid);
        const auto& S=workers[k].S;
        for (size_t i=1; i<S.size(); ++i) {
            if (S[i]&1) {       // dropoff
                rt.insert(Stop(requests[S[i]>>1].did, Stop::Type::dropoff,
                        requests[S[i]>>1].req_id));
            } else {            // pickup
                rt.insert(Stop(requests[S[i]>>1].oid, Stop::Type::pickup,
                        requests[S[i]>>1].req_id));
            }
        }
        routes.push_back(move(rt));
    }
    cout<<"tot_cost by FESI: "<<tot_cost<<endl;
    return routes;
}
...
// Florio: adapted to return distance/cost based on Instance object
...
// Florio: setup parameters based on Instance object
void loadInstance() {
    nV=I->nodes();
    nW=I->drivers().size();
    nR=I->requests().size();
    ...
    for (int i=0; i<nW; ++i) {
        workers[i].oid=I->drivers()[i].s;
        workers[i].cap=I->capacity();
    }
    for (int j=0; j<nR; ++j) {
        requests[j].oid=I->requests()[j].s;
        requests[j].did=I->requests()[j].t;
        requests[j].req_id=I->requests()[j].id;
        requests[j].wei=1;
    }
}

// Florio: load HST from file
void readHSTFromFile(string hstFileName) {
    // (4)
